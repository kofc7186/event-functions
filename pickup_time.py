    ...
    if result.is_success():
        for order in result.body['orders']:
            ...
            for item in order['line_items']:
                if item['name'] == "Fried Fish" and (min_time == "" or item['variation_name'] < min_time):
                    min_time = item['variation_name']
            if min_time == "":
                min_time = "5:30PM-5:45PM"
            #print(f"update orders set pickup_window = '{min_time}' where id = '{order['id']}';")
            print(f"{min_time}")
    else:
